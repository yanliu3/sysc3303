...
public class TFTPSim {
	...
	public void InvalidRRQ(){
		...
		for(;;) { // loop forever
			...
			if (error[1] == 1){
				...
				try{
					sendPacket = new DatagramPacket(error, error.length, InetAddress.getLocalHost(), serverPort);
		...
		// Send the datagram packet to the server via the send/receive socket.
			...
			sendSocket.close();
			System.out.println("Enter 'quit', if you want to go back to menu. Otherwise, enter 'ok'.");
			quit = sc.next();
			if(quit.equals("quit")){ quit = ""; break;}
		} // end of loop
	}
	...
	public void UnknownID () {
		...
		for(;;) { // loop forever
			...
			try{
				sendPacket = new DatagramPacket(data, receivePacket.getLength(), InetAddress.getLocalHost(), 70);
			}catch (UnknownHostException e) {
			     e.printStackTrace();
			     System.exit(1);
			}
	...
	public void delayPacket(){
		...
		if(choice == 1){
			...
			for(;;) { // loop forever
				...
				FileCopier.printReceivePacket(receivePacket, "Sim", true);
				// Process the received datagram.		
				sendPacket = new DatagramPacket(receivePacket.getData(), receivePacket.getLength(), ip, clientPort);
